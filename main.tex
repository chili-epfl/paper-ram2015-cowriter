...
the child practises hand-writing but, as (s)he takes on the role of the teacher,
...
\label{studies}
\footnotesize
\begin{tabular}{@{}lp{4cm}p{2.2cm}p{1.8cm}l@{}}
\toprule
...
{\it Vincent}     & Case-study, spanning over 4 weeks              & 1.5 hours $\times$ 4 weeks & 1                                     & 5          \\
...
The CoWriter activity needs a child engaged as interaction leader. In this
study we consider the problem of long-term interactions : is it possible to sustain this
engagement over several one-hour sessions ?\\

Vincent is a six years old child with troubles in handwriting and a poor
self-esteem. Our goal was to figure out an environment in
order to make Vincent sustaining engagement over four sessions of one hour, one
session per week. We decided to introduce an appealing scenario that justifyed to the child
the activity where a robot wants to learn handwriting. We used two Nao robots :
a blue one (called Mimi) and an orange one (called Clem). Mimi was away for a
mission,and the two robots had to communicate by mails. But they decided to do
it ``like humans", with handwritten messages. While Mimi was a good in
handwriting, Clem had strong difficulties and needed the help of Vincent.\\

During the first session, Vincent just teached single letters to the robot.
To chose a letter, he had to pick it in a set of magneted plastic letters, and to
show it to the robot. But the robot was not programmed to detect and recognize
it. A third person was entering the letter and was sending it to the
system with the computer. With following sessions we gradually increased the
difficulty : in the second session the child teached full words. To chose the
word, he had to assemble plastic letters to build it and to show it to the
robot. In the two last sessions, he teached long sentences. This time he just
had to say the sentence to the robot, and then the third person was using the
compurter to enter it. \\

We designed a special scenario for the last session : Mimi came back to the
laboratory. In front of Vincent, Mimi said to Clem that it did not believe 
it realy improved its handwritting, and that the responses it recieved
was written by Vincent. To prove that it actualy improved its writing,
it gave to Clem thirty minutes for a final training. After the training, it
asked to Clem to write some words. Finally, he admitted that Clem actually
improved its handwriting.

\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{diego}
    \caption{\small Vincent working with Nao}
    \label{fig:diego}
\end{figure}


\subsubsection{Results}
In all sessions, Vincent was focused on the activity during forty minutes up to
one hour. In total, he provided 154 letters as demonstration.\\

Vincent accepted our scenario and seriously played the game. At the
beginning he seemed not confident and intimidated.  But quickly, he gain in
confidence to play with the robot. He even showed real affective bonds : he
sent an handwritten mail to it some days after the last session.\\

As a technical result, we do not know if Vincent improved his handwriting but he
likely conviced himself he was good enough at writing to help someone else (the
robot in that case). To show to Vincent how much he improved the robot's
handwriting, we printed the letter that the robot wrote during the third session
before and after his help (Fig.~\ref{fig:stimuli}).
...
\subsubsection{Context, Study Design}
Thomas is a five and half years old child. It is followed by an occupational
...
What is more, Thomas is strongly careless : he rarely payed attention to
...
(visuo-constructive deficits and inattention) in order to keep Thomas focused
...
sessions as well. This time, we assumed that a scenario like the one we used for Vincent was
no longer relevant with Thomas. We just introduced the robot and quickly
...
\subsubsection{Results}
Despite his inattention, Thomas was able to keep on the activity during more than
forty minutes in each session.\\

As soon as Thomas understood that the robot was only accepting recognizable
...
accepted by the robot. According to the therapist, it was the first time Thomas
was intrinsically correcting himself.\\

Since the robot was learning from scratch, each time Thomas succeeded to make his
demonstrations accepted by it, the improvement of its handwriting was
obvious~\ref{learning_6_demos},~\ref{henry_distances}.
Therefor, Thomas quickly enter in an self-rewarding game where the rewards were
the improvement of the robot. Over the three last sessions he provided 99
...
and Thomas), the educational scenarios were also always designed and monitored by
the adults.
